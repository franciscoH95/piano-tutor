...
- [ ] T042 [US1] **Spike de validación en máquina Windows real. BLOQUEADO: no hay máquina Windows disponible hoy.** La rama de Windows (T042–T044, T067) queda **suspendida**, no en curso: no se empieza y no cuenta como pendiente de trabajo, sino como pendiente de hardware. El resto de fases avanza sin ella. Es el **primer** trabajo del lado Windows y bloquea a los demás. Validar en este orden: enumeración y apertura; `CM_Register_Notification`; cierre tras retirada del dispositivo **sin cuelgue** (Microsoft KB4460006 documenta uno irrecuperable); y una cifra de latencia. Registrar los hallazgos en `specs/002-midi-input-capture/research.md` bajo las incertidumbres U2 y U4. **2026-08-19**: el arnés para hacerlo ya existe y es portable — `cargo run -p piano-midi-io --example escuchar` recorre los cinco puntos en orden y da un veredicto por paso; el procedimiento está en `quickstart.md`. Hasta esa fecha el ejemplo y el banco tenían el cuerpo bajo `#[cfg(target_os = "macos")]`, así que la tarea estaba bloqueada por algo más que la falta de máquina
- [X] T043 [US1] Implementar enumeración, apertura, bucle de mensajes y cierre en `midi-io/src/windows.rs` con ~~WinMM~~ **WinRT** (`Windows.Devices.Midi`), aplicando lo aprendido en T042. **Enmendado el 2026-08-18**: toda la API de WinMM en el crate `windows` es `pub unsafe fn`, y este crate tiene `forbid(unsafe_code)`, que no se levanta con un `allow` local. Ver `research.md`, Decisión 1
...
- [X] T067 [US4] Implementar el vigía de Windows en `midi-io/src/vigia.rs` con ~~`CM_Register_Notification` y `GUID_DEVINTERFACE_MIDI_INPUT`~~ **sondeo portable**, aplicando lo aprendido en T042. **Enmendado el 2026-08-18**: el vigía acabó siendo el mismo en las dos plataformas —sondea la enumeración cada segundo— y por eso no lleva ni un `#[cfg]`. `CM_Register_Notification` también es `unsafe` en el crate `windows`; y el sondeo funciona en cualquier hilo, sin la dependencia de orden de arranque que exigen las notificaciones
