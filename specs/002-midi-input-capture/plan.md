...
| ~~La deuda del Principio IV~~ **SALDADA el 2026-08-18** | — | El banco existe, corre sin teclado, falla al superar el umbral, se ejecuta en integración continua en macOS y Windows, y desde que el repositorio es público las dos comprobaciones son **obligatorias** en `main`, con force-push y borrado de rama deshabilitados. La deuda que la feature 001 dejó anotada queda cerrada. **Matiz que no conviene olvidar**: `enforce_admins` está en `false`, así que quien administra el repositorio puede saltarse la puerta a sabiendas, igual que puede saltarse el hook local con `--no-verify`. La puerta protege de los descuidos, no de las decisiones deliberadas, y eso es todo lo que una puerta puede hacer. |
