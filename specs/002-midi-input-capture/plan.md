...
|-----------|------------|-------------------------------------|
| Una porción del código —la que habla con la API MIDI del sistema operativo— no queda cubierta por pruebas automáticas | Ningún runner de integración continua tiene un piano enchufado. Esa capa solo se puede ejercer a mano, con hardware delante. | Simular el sistema operativo entero fue descartado: se acabaría probando el simulador, no el sistema. La mitigación es estructural, no de esfuerzo: esa capa se reduce a la mínima expresión —abrir el puerto y reenviar bytes— sin ninguna decisión propia, de modo que lo que queda sin cubrir sea trivial y lo que tiene lógica esté todo del lado probado. El plan debe declarar qué archivos son esa capa y mantenerlos deliberadamente pequeños. |
